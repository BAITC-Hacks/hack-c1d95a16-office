...
describe('API integration',()=>{
 ...
 vi.stubGlobal('fetch',vi.fn((url:string)=>Promise.resolve(reply(url.endsWith('/health')?{status:'ok',simulation_engine:'available'}:url.endsWith('/bootstrap')?{datasetVersion:'sim-test',budget:100}:url.endsWith('/baseline')?{...wire,total_cost:0,remaining_budget:100,score:wire.baseline_score,score_delta:0,city_average:56.8624,critical_count:2}:url.endsWith('/districts')?demoCityData.districts.map(d=>({name:d.id,population_share:d.share,indicators:d.indicators})):demoCityData.actions.map(a=>({...a,name:a.title,category:category[a.category],scope:a.scope.toUpperCase()}))))));
 const city=await loadCityData();expect(city.mode).toBe('backend');expect(city.datasetVersion).toBe('sim-test');expect(city.baseline?.baselineScore).toBe(wire.baseline_score);expect(city.actions).toHaveLength(14);
 });
 ...
 it('preserves mixed per-measure district assignments',()=>expect(toBackendPlan({...backendPlan,decisions:DEMO_PLAN})).toEqual(toWirePlan(DEMO_PLAN)));
 it('does not send client-computed scores or require a catalog version',()=>expect(toBackendPlan({decisions:DEMO_PLAN})).toEqual(toWirePlan(DEMO_PLAN)));
 ...
 it('posts explicit decisions to simulate and preserves the engine result',async()=>{const fetch=vi.fn().mockResolvedValue(reply(wire));vi.stubGlobal('fetch',fetch);const result=await simulate(backendPlan,'backend');expect(result.projectedScore).toBe(wire.score);expect(fetch.mock.calls[0][0]).toMatch(/\/simulate$/);expect(JSON.parse(fetch.mock.calls[0][1].body)).toEqual(toWirePlan(backendPlan.decisions));});
 it('posts explicit decisions to AI analyze and preserves missing-key status',async()=>{const fetch=vi.fn().mockResolvedValue(reply({simulation:wire,analysis:null,aiStatus:{status:'configuration_error'}}));vi.stubGlobal('fetch',fetch);const result=await analyze(backendPlan,'backend');expect(result.simulation?.projectedScore).toBe(wire.score);expect(result.analysis).toBeNull();expect(fetch.mock.calls[0][0]).toMatch(/\/ai\/analyze$/);expect(JSON.parse(fetch.mock.calls[0][1].body)).toEqual(toWirePlan(backendPlan.decisions));});
 ...
 it('allows the AI council enough time for deterministic optimization',async()=>{vi.useFakeTimers();vi.stubGlobal('fetch',vi.fn((_url,init)=>new Promise((_resolve,reject)=>init.signal.addEventListener('abort',()=>reject(new DOMException('Aborted','AbortError'))))));const task=expect(analyze(backendPlan,'backend')).rejects.toThrow('180 секунд');await vi.advanceTimersByTimeAsync(180001);await task;});
