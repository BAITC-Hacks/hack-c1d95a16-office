...
export function parseCityData(districtRaw:unknown,measureRaw:unknown,baselineRaw:unknown):CityData {
  const districts=districtCatalogSchema.parse(districtRaw),measures=measureCatalogSchema.parse(measureRaw),baseline=parseSimulation(baselineRaw);
  ...
  if(baseline.spent!==0||baseline.remaining!==100) throw new Error('Бастапқы бюджет 100, шығын 0 болуы керек.');
  return {mode:'backend',budget:baseline.remaining,budgetUnit:'бірлік',baseline,
    districts:districtIds.map(id=>{const d=districts.find(d=>d.name===id)!;return {id,name:districtNames[id],description:'Backend қайтарған бастапқы жағдай',share:d.population_share,baselineD:baseline.districtResults.find(d=>d.id===id)!.beforeD,indicators:d.indicators};}),
...
const councilSchema=z.object({
  ...
  risk:z.object({risk_level:z.string(),risks:strings,critical_findings:strings,warnings:strings}),
...
export function parseOptimization(raw:unknown):OptimizationResult {
  const r=z.object({simulation:summarySchema,scenarios:z.array(summarySchema).min(1),recommended:z.object({improvement:n}),comparison:z.object({total_cost_delta:n,critical_count_delta:n}).optional()}).parse(raw);
  return {current:summary(r.simulation),recommended:summary(r.scenarios[0]),improvement:r.recommended.improvement,costDelta:r.comparison?.total_cost_delta??null,criticalDelta:r.comparison?.critical_count_delta??null};
}
