...
import { ArrowDown, ArrowRight, ArrowUpRight, Building2, Check, CheckCircle2, CircleHelp, Compass, FlaskConical, LayoutDashboard, Leaf, LoaderCircle, Map, RefreshCw, RotateCcw, Sparkles, X, AlertCircle, ChevronRight } from 'lucide-react';
...
import { analyze, canAnalyze, canSimulate, isDemo, loadCityData, simulate } from './api/client';
import { selectedActions, totalCost, trySelect } from './lib/budget';
import { categories, type Action, type AnalysisResult, type CategoryId, type CityData, type Selection, type SimulationResult } from './types';
...
export function Dashboard() {
  ...
  useEffect(() => () => { simulationRequest.current?.abort(); analysisRequest.current?.abort(); }, []);
  ...
  function invalidate() {
    ...
    simulationRequest.current?.abort(); analysisRequest.current?.abort();
    setSimulating(false); setAnalyzing(false); setResult(null); setAnalysis(null);
    setSimulationError(null); setAnalysisError(null);
  }
  function reset() {
    invalidate(); setSelection({}); setDistrictId(data?.districts[0]?.id ?? '');
    setExpanded('transport'); setNotice('Сценарий бастапқы күйге қайтарылды.');
  }
  function selectDistrict(id: string) {
    if (id === districtId) return;
    invalidate(); setDistrictId(id); setSelection({}); setExpanded('transport');
    setNotice('Аудан өзгерді. Жаңа аудан үшін бес шешімді таңдаңыз.');
  }
  function selectAction(action: Action) {
    if (!data || selection[action.category] === action.id) return;
    const next = trySelect(selection, action, data.actions, data.budget, districtId);
    if (!next) { setNotice('Шара бюджетке сыймайды немесе бұл ауданға қолжетімсіз.'); return; }
    invalidate(); setSelection(next); setNotice(`${action.title} таңдалды.`);
  }
  function removeAction(id: CategoryId) {
    invalidate(); const next = { ...selection }; delete next[id]; setSelection(next);
    setNotice('Таңдау алынып тасталды. Бюджет жаңартылды.');
  }
  async function calculate() {
    if (!data || !complete || !canSimulate || spent > data.budget) return;
    ...
    try {
      const next = await simulate({ datasetVersion: data.datasetVersion, districtId, actionIds: chosen.map(a => a.id) }, controller.signal);
      ...
      setResult(next); setNotice('Есеп дайын. Енді AI талдауын сұратуға болады.');
    } catch (error) {
  ...
  async function requestAnalysis() {
    if (!result || !canAnalyze || analyzing) return;
    ...
    try {
      const next = await analyze(result, controller.signal);
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb"><Building2 size={15}/><span>Астана</span><ChevronRight size={13}/><strong>Қала симуляторы</strong></div><div className="topbar-right"><span className={`mode-badge ${isDemo ? '' : 'live'}`}>{isDemo ? <FlaskConical size={13}/> : <CheckCircle2 size={13}/>} {isDemo ? 'Демо деректер' : 'Backend режимі'}</span><span className="language-label">ҚАЗ</span><span className="avatar" aria-label="Әкім рөлі">Ә</span></div></header>
      <main id="main">
        <section className="page-intro" id="overview"><div><div className="intro-kicker"><span/> ҚАЛАНЫ БАСҚАРУ СИМУЛЯТОРЫ</div><h1>Қала болашағы —<br className="mobile-break"/> <em>сенің шешімің.</em></h1><p>Бір бюджет. Бес бағыт. Ортақ болашақ.</p></div><button type="button" className="secondary-button" onClick={reset} disabled={!data || loading}><RotateCcw size={16}/>Қайта бастау</button></section>
        ...
        : data && <>
          ...
          {isDemo && <div className="demo-notice"><FlaskConical size={16}/><p><strong>Демо деректер.</strong> Аудандар, көрсеткіштер мен бағалар — интерфейсті тексеруге арналған шартты мысалдар.</p><button type="button" onClick={() => help.current?.showModal()} aria-label="Демо режимі туралы"><CircleHelp size={17}/></button></div>}
          <DistrictCards districts={data.districts} selectedId={districtId} onSelect={selectDistrict}/>

          <div className="workspace-grid">
            <section id="decisions" className="decisions-section"><div className="section-heading"><div><span className="step-label">02 · СЕНІҢ ЖОСПАРЫҢ</span><h2>Бес бағыт. Бес шешім.</h2></div><span className={`count-badge ${complete ? 'done' : ''}`}>{complete && <Check size={13}/>}<span data-testid="decision-count">{chosen.length}</span> / 5</span></div>
              <p className="section-description"><span className="district-pill"><Map size={12}/>{district?.name}</span> Әр бағыттан бір шара таңдаңыз.</p>
              <DecisionSelector actions={data.actions} selection={selection} budget={data.budget} unit={data.budgetUnit} districtId={districtId} expanded={expanded}
                onExpand={id => setExpanded(expanded === id ? null : id)} onSelect={selectAction} onRemove={removeAction}/>
              <div className="submit-panel"><div className="completion-indicators" aria-label={`${chosen.length} бағыт таңдалды`}>{categories.map(category => { const Icon = categoryIcons[category.id]; return <span key={category.id} className={selection[category.id] ? 'complete' : ''} title={category.name}><Icon size={15}/></span>; })}<span>{complete ? 'Бес шешім дайын' : `${5 - chosen.length} бағыт қалды`}</span></div>
                <button className="primary-button calculate-button" type="button" disabled={!complete || !canSimulate || simulating} onClick={calculate}>{simulating ? <LoaderCircle className="spin" size={16}/> : <Sparkles size={16}/>} {simulating ? 'Есептелуде…' : 'Нәтижені есептеу'}<ArrowRight size={17}/></button>
                {!canSimulate && <p className="service-note">Таңдауларды тексеруге болады. Нақты нәтиже үшін есептеу сервисі қосылуы керек.</p>}
                {simulationError && <p className="inline-error" role="alert">{simulationError}</p>}
              </div>
              <p className="district-change-note">Ауданды ауыстыру немесе «Қайта бастау» таңдауларды тазартады.</p>
            </section>
            <aside className="results-column" aria-label="Сценарий нәтижелері"><div className="results-column-heading"><span className="step-label">03 · ШЕШІМДЕРДІҢ ӘСЕРІ</span><ArrowDown size={14}/></div>
              <ScorePanel result={result} connected={canSimulate} loading={simulating}/>
              <AIAnalysisPanel analysis={analysis} loading={analyzing} error={analysisError} available={Boolean(result && canAnalyze)} onAnalyze={requestAnalysis}/>
    ...
    <dialog className="help-dialog" ref={help} aria-labelledby="help-title"><button type="button" className="dialog-close" onClick={() => help.current?.close()} aria-label="Жабу"><X size={21}/></button><span className="icon-box soft"><Building2 size={25}/></span><h2 id="help-title">5 сағатқа әкім</h2><p>Қаланың бес бағыты бойынша шектеулі бюджетпен шешім қабылдауға арналған симулятор.</p><ol><li>Жоспар ауданын таңдаңыз.</li><li>Әр бағыттан бюджетке сай бір шара таңдаңыз.</li><li>Есептеу сервисі қосылғанда нәтижені алыңыз.</li><li>AI талдауынан шешімдердің түсіндірмесін оқыңыз.</li></ol><div className="dialog-note">Қазіргі демо мәліметтер ресми датасет емес. Frontend Score есептемейді және AI жауабын ойдан шығармайды.</div><button className="primary-button" onClick={() => help.current?.close()}>Түсінікті<Check size={16}/></button></dialog>
  </div>;
}
