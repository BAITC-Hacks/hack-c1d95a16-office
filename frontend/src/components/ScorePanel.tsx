...
export function ScorePanel({result,baseline,loading,demo}:{result:SimulationResult|null;baseline:SimulationResult|null;loading:boolean;demo:boolean}){
  return <section className="panel score-panel" id="results" aria-busy={loading}>
    ...
    </>:<div className="empty-score"><span className="score-placeholder">{baseline?number(baseline.baselineScore):'—'}</span><strong>{loading?'Сервер есептеп жатыр…':demo ? 'Бастапқы Score · демо' : baseline?'Бастапқы Score · Backend':'Есеп серверден күтілуде'}</strong><p>Тура 5 шара таңдап, симуляцияны іске қосыңыз.</p>{loading&&<LoaderCircle className="spin" size={18}/>}</div>}
  </section>;
}
