...
export interface District {id:string;name:string;description:string;share:number;baselineD:number|null;indicators:Record<IndicatorId,number>}
export interface Decision {actionId:string;districtId:string|null}
export type Selection=Decision[];
export interface Plan {decisions:Selection;datasetVersion?:string}
...
export interface SimulationResult {analysis?:AnalysisResult;spent:number;remaining:number;baselineScore:number;projectedScore:number;delta:number;cityAverage:number;weakestDistrict:string;criticalCount:number;districtResults:DistrictResult[]}
export interface CityData {mode:'demo'|'backend';datasetVersion?:string;fallbackReason?:string;budget:number;budgetUnit:string;districts:District[];actions:Action[];baseline:SimulationResult|null}
...
export interface AnalysisResult {simulation?:SimulationResult;analysis:Council|null;status:string;message:string|null}
...
export interface OptimizationResult {current:ScenarioSummary;recommended:ScenarioSummary;improvement:number|null;costDelta:number|null;criticalDelta:number|null;reasoning?:string|null;aiStatus?:string}
